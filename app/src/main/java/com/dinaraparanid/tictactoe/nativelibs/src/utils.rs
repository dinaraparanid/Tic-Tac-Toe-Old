...
#[inline]
pub(crate) fn get_struct<'a, T>(env: *mut JNIEnv, pointer_buffer: jobject) -> Option<&'a T> {
    unsafe {
        ((**env).GetDirectBufferAddress.unwrap_unchecked()(env, pointer_buffer) as *mut T).as_ref()
    }
}

#[inline]
pub(crate) fn get_struct_mut<'a, T>(
    env: *mut JNIEnv,
